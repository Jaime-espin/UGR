...
int main( int argc, char *argv[] )
{
   ...
   while( true ) // ciclo principal
   {
      ...
      for( int i = 1 ; i <= 4 ; i++ ) // ciclo secundario (4 iteraciones)
      {
         ...
         // Obtener el instante real actual tras la espera
         time_point<steady_clock> fin_real_sec = steady_clock::now();
         
         // Calcular el retraso: diferencia entre el instante real y el esperado
         milliseconds_f retraso_ms = fin_real_sec - ini_sec;
         
         // Informar del retraso al final del ciclo secundario
